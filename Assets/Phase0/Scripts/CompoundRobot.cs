...
public class CompoundRobot : MonoBehaviour
{
    ...
    void UpdateRamEffort()
    {
        ...
        // C6.1: a tipped-over hull is not driving an attack. Wheels can still
        // touch and spin while the hull lies on its side (the tipper's whole
        // failure mode), which kept ramEffort alive and billed the FALLEN
        // tower for impulses its opponent generated by ramming the carcass
        // (slam ledger: J=3000-4500 'attacks' at up.y 0.3, speed 0.1 m/s).
        // 0.45, not 0.7 (HazardBench regression): hammer knocks and wedge
        // scrambles tilt FIGHTING bots past 45 deg routinely, and at 0.7 the
        // L3-L5 hazard fights billed zero mutual damage (env 496 = taken 496).
        // The fallen-carcass case this gate exists for sits at up.y ~0.3.
        if (transform.up.y < 0.45f) return;
    ...
    // ---- C6.1 (tipper tune): GRAVITY SLAMS ARE NOT RAMS. A toppling robot
    // (the tipper's whole act) that is still mid-charge when it falls keeps a
    // live ramEffort, so the causal split billed the full gravity-inflated
    // impulse as a combat ram: 500-1000 HP + all four wheels sheared per fall
    // at L1/L2 (C5 round-6/8 diag), from the bot whose blurb promises it beats
    // itself. Robot-on-robot impulses whose VERTICAL fraction exceeds
    // SLAM_VERT_FRAC blend down to SLAM_DMG_SCALE - applied to the HP impulse
    // AND the seam stress, both sides symmetrically (each body's own callback
    // sees the mirrored impulse). Horizontal rams and wedge lifts stay below
    // the 0.6 threshold; hazards use ApplyHit directly and never pass here.
    public static float SLAM_VERT_FRAC = 0.6f;
    // C6.1 diag: per-hit ledger of robot-vs-robot HP bills (bench only).
    public static bool slamLogOn = false;
    public static string slamLog = "";
    public static float SLAM_DMG_SCALE = 0.35f;
    ...
    void Accumulate(Collision c)
    {
        ...
        float slamScale = 1f;
        float vertFracDbg = 0f;
        if (fromRobot)
        {
            float vertFrac = Mathf.Abs(c.impulse.y) / Mathf.Max(J, 1e-4f);
            vertFracDbg = vertFrac;
            if (vertFrac > SLAM_VERT_FRAC)
                slamScale = Mathf.Lerp(1f, SLAM_DMG_SCALE,
                    (vertFrac - SLAM_VERT_FRAC) / (1f - SLAM_VERT_FRAC));
        }
        ...
        float Js = SeamLoad(J) * (fromRobot ? structMul * slamScale
        ...
        if (fromRobot && !dead && J > DamageResolver.RAM_MIN_J)
        {
            ...
            if (vIdx >= 0)
            {
                ...
                if (causal > 0f)
                {
                    if (slamLogOn && J * slamScale * causal > 250f && slamLog.Length < 6000)
                        slamLog += attacker.name + ">" + name
                            + " J=" + J.ToString("F0")
                            + " vf=" + vertFracDbg.ToString("F2")
                            + " sc=" + slamScale.ToString("F2")
                            + " ca=" + causal.ToString("F2")
                            + " hd=" + hardness.ToString("F2")
                            + " aUp=" + attacker.transform.up.y.ToString("F2")
                            + " aSpd=" + VelUtil.GetLinearVelocity(attacker.rb).magnitude.ToString("F1") + "\n";
                    DamageResolver.RamHit(attacker, hardness, this, vIdx, J * slamScale, p, causal);
