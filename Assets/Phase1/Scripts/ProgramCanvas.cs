...
public class ProgramCanvas : MonoBehaviour
{
    ...
    void BuildUI()
    {
        ...
        // V2.4: the three preset buttons merged into one STARTER KIT (the
        // deduped union — RobotProgram.StarterKit()). The old preset methods
        // survive for benches and the balance matrix.
        MkChip("preset_starter", actions.transform, "STARTER KIT", 11f,
               new Color(0.15f, 0.22f, 0.30f, 0.95f), () => InstallPreset(RobotProgram.StarterKit()), 132f);
        ...
        // V2.5 (owen): the DOCUMENT actions live at the BOTTOM of the
        // expanded view — SAVE / SAVE AS write the program library
        // (Career.Data.programs); LOAD TO ROBOT is what arms the canvas
        // program on the open machine (the old top-SAVE behavior).
        var docBar = MkRow(transform);
        docBar.name = "progbottom";
        MkChip("prog_open", docBar.transform, "OPEN", 12f,
               new Color(0.18f, 0.24f, 0.32f, 0.95f), OpenPicker, 84f);
        MkChip("prog_save", docBar.transform, "SAVE", 12f,
               new Color(0.16f, 0.34f, 0.18f, 0.95f), OnSaveTapped, 84f);
        MkChip("prog_saveas", docBar.transform, "SAVE AS", 12f,
               new Color(0.15f, 0.22f, 0.30f, 0.95f), OpenSaveAs, 100f);
        MkChip("prog_load", docBar.transform, "LOAD TO ROBOT", 12f,
    ...
    void BuildOpenDlg()
    {
        ...
        v.spacing = 6f; v.padding = new RectOffset(10, 10, 10, 10);
        v.childForceExpandWidth = true; v.childForceExpandHeight = false;
        MkText("t", openDlg.transform, "OPEN PROGRAM", 12f, TextAnchor.MiddleLeft);
    ...
    void RefreshOpenList()
    {
        ...
        for (int i = 0; i < lib.Count; i++)
        {
            ...
            string label = e.name + (back != null ? "  (" + back.hats.Count + " hats)" : "  (unreadable)");
