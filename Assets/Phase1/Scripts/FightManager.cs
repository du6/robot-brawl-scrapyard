...
public class FightManager : MonoBehaviour
{
    ...
    bool TickCountOut(Side s, float dt)
    {
        ...
        if (VelUtil.GetLinearVelocity(s.bot.rb).magnitude < 0.3f) s.immobileTime += dt;
    ...
    string SideDetail(Side s)
    {
        ...
        return string.Format("      seams sheared {0} · flipped {1:F0}% of the match · immobile {2:F0}% · gyros {3} · battery {4}",
            ...
            s.gyro == null ? "none fitted" : (s.gyro.LiveGyros() + " live"), pwr);
    }
    ...
    void BigLine(string text, Color c, float yFrac)
    {
        ...
        GUI.Label(new Rect(3f, y + 3f, Screen.width, 60f), text, medStyle);
        medStyle.normal.textColor = c;
        GUI.Label(new Rect(0f, y, Screen.width, 60f), text, medStyle);
    }
