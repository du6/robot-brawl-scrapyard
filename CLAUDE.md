...
| doc | what |
|---|---|
| `docs/Scraplands_Map_Mode_Design_2026-09-09.md` | **the map as the front door (owen, design only)** — drive SCRAPPER round a seeded yard, crates are `qbox`es, parked robots (yard bots + an anonymous pool of real snapshots, no programs) are Quick bouts under an auto-brain picked by the build; rewards local, POINTS only from the referee; login only to send a challenge. Branch `feature/scraplands`, all new files, M0 is a one-week prototype with a stop rule |
