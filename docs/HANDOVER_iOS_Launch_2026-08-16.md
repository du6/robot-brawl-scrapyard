...
| Deployed | **`rb-api-00025-vsg`, 2026-08-19** — parts can no longer be sold back. `POST /v1/economy/purchase` refuses `op=sell`; verified on production with the authorised test account (a refusal writes nothing), and an unknown op still answers the old "op must be buy or sell", so the two failures stay distinguishable. ⚠ **There is now NO way to change a part's material** — REWORK went on 08-05 and SELL+BUY was the last route |
| Deployed (prev) | **`rb-api-00024-4nq`, 2026-08-19** — adds `POST /v1/robots/{id}/retire` (there was no way to remove an enlisted robot; the only thing that ever set `robots.retired` was account deletion) and migration **015**, which makes a display name globally unique and case-insensitive. 015 DEDUPES rather than asserting: oldest account keeps a contested name, later ones become `<base>~<4 hex>`, and every rename is in `display_name_migration`. Verified after deploy: **no board owner was renamed** — 25 entries, identical owner names before and after; "The Yard" is one account with ten robots, not ten accounts |
