...
app.MapPost("/v1/snapshots", async (SnapshotReq req, ClaimsPrincipal user, IBlobStore blobs) =>
{
    ...
    try
    {
        ...
        sha = doc.RootElement.GetProperty("sha256").GetString() ?? "";
    ...
    async (long id, HttpContext ctx, JobQueue q, ValidateResult res) =>
{
    ...
    // §1.2: one ACTIVE snapshot per robot. The new one supersedes the old in
    // the same transaction, so there is never a moment with two or none.
    if (res.Legal)
    {
