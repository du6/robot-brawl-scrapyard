...
public class RobotProgram
{
    ...
    /// <summary>V2.8 (critic loop 7, F1b) — THE WHOLE SHOPPING LIST.
    /// Validate stops at the FIRST failing block, which is right for a
    /// validator and wrong for a player: "needs a Wall sensor" sends them to
    /// the shop, they buy it, come back, and are told about the next one.
    /// Three round trips to learn one sentence. This walks every hat and
    ...
    /// battery and four wheels — and not one sensor — so the sensor-heavy
    /// STARTER KIT program cannot load on the day a career is created. This
    /// one references no sensor at all: it is pure blind driving, and its
    /// only hardware claim is the wheels already in the crate. It is the
    /// on-ramp; STARTER KIT is the thing you graduate to.</summary>
    public static RobotProgram FirstSteps()
    {
        var p = new RobotProgram { title = "First Steps" };
        var h = new PHat { name = "CHARGE" };
        h.note = "Runs on the parts you started with — no sensors needed. "
               + "Drive, turn, repeat: a ram-bot that never stops moving. "
               + "Buy a sensor and the STARTER KIT program opens up.";
        h.when.Add(PCondTerm.Always());
        h.body.Add(PBlock.MkForever());
        h.body.Add(PBlock.MkMove(70f, 1.2f, 0));
        h.body.Add(PBlock.MkTurnLR(60f, 0.5f));
        h.body.Add(PBlock.MkEnd());
        p.hats.Add(h);
        return p;
    }
