...
public class ProgramRunner : MonoBehaviour
{
    ...
    const float BRAKE_TIME  = 0.20f;   // s of counter-thrust after a timed step
    const float BRAKE_POWER = 0.75f;
    ...
    void AbandonHat(bool disarm)
    {
        if (stepEntered && activeHat >= 0 && activeHat < program.hats.Count)
        {
            ...
            if (pc >= 0 && pc < body.Count) ExitStep(body[pc]);
        }
        ZeroDrive();
    ...
    void Advance(float dt)
    {
        ...
        // INSTANT BUDGET EXHAUSTED. This hat ran 64 steps inside one physics
        ...
        int lower = -1;
        for (int i = activeHat + 1; i < program.hats.Count; i++)
            if (CondsTrue(program.hats[i].when)) { lower = i; break; }
        ReleaseHat(lower < 0);
    ...
    void BeginBrake()
    {
        ...
        float yaw = rb.angularVelocity.y;                  // rad/s, +y swings left
        if (Mathf.Abs(fwd) >= BRAKE_STOP) brakeFwd   = -Mathf.Sign(fwd) * BRAKE_POWER;
        if (Mathf.Abs(yaw) >= BRAKE_YAW)  brakeSteer =  Mathf.Sign(yaw) * BRAKE_POWER;
        if (brakeFwd != 0f || brakeSteer != 0f) brakeUntil = clock + BRAKE_TIME;
    }
    ...
    void ExitStep(PBlock b)
    {
        if (b.op == POp.RunMotor) ZeroMotor(b);       // "ran for n" ⇒ stops after n
            ...
            BeginBrake();       // timed macros STOP -- coasting is not stopping
    }
    ...
    void ApplyChannels()
    {
        ...
        else
        {
            ...
            if (!idle) { brakeUntil = 0f; brakeFwd = 0f; brakeSteer = 0f; }
        }
