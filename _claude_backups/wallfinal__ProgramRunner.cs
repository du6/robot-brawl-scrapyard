...
public class ProgramRunner : MonoBehaviour
{
    ...
    void RelCommand(int target, float pct, out float drive01, out float steer)
    {
        if (pct < 0f && target == (int)PTarget.Wall && bus != null && bus.wallValid)
        {
            ...
            // controller on a low-friction mass, and it does what those
            // always do: it sails past the set point and comes back. The
            // first measurement of the field without this term settled into a
            // 3.98-6.59 m swing with 3 command reversals in 8 s -- far better
            // than the 1.02-6.91 m wall-to-wall cycle it replaced, but still
            // hunting. So subtract the retreat we are ALREADY making: thrust
            // buys only the difference between the speed we want and the
            // speed we have. Band 1.27 m, one reversal in 8 s, peak 2.58 m/s.
            //
            // CLAMPED AT ZERO, not allowed negative. Letting it go to -0.6 as
            // an active brake read better on paper and immediately tripped
            // owen's own regression check: with the wall DEAD AHEAD the
            // escape is astern, so a negative term flips the command to +0.17
            // FORWARD -- thrust into the wall, which is the exact bug he
            // reported. Coasting is a slower brake and it is the honest one.
            float u = Mathf.Clamp01(gov - EscapeSpeed() / WALL_TOP_SPEED);
    ...
    /// direction. Negative means we are still closing on the arena edge.</summary>
    float EscapeSpeed()
    {
        var rb = self != null ? self.GetComponent<Rigidbody>() : null;
        if (rb == null || bus == null) return 0f;
        Vector3 fwd = self.transform.forward; fwd.y = 0f;
        if (fwd.sqrMagnitude < 1e-6f) return 0f;
        Vector3 dir = Quaternion.AngleAxis(bus.wallEscapeDeg, Vector3.up) * fwd.normalized;
        return Vector3.Dot(rb.linearVelocity, dir);
    }
